...
-(BOOL)findUsersByname:(NSString *)name
{
    NSLog(@"findUsersByName");
    return [[WZNetworkHelper helper] help:[self class] with:@{@"userName":name} object:nil by:RKRequestMethodGET];
...
- (BOOL)findAllUsers
{
    ...
    return [[WZNetworkHelper helper] help:[self class] with:@{@"maxDistance":@"1000000",@"location":location} object:nil by:RKRequestMethodGET];
}


- (BOOL)updateLocation:(NSString *)newLocale
{
        NSMutableDictionary *changesWithID = [NSMutableDictionary new];
    
        [changesWithID setObject:newLocale forKey:kWZLocationKey];
...
- (BOOL)update
{
    NSDictionary *changes = [WZProfileModifier modifier].changes;//复制后看原对象与新对象是否一至
    if (changes) {
