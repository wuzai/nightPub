...
{
    [[WZKeychain keychain] prepareForUsername:username password:password];//先准备好用户名密码
...
+ (void)succeedIn:(RKObjectLoader *)loader withResults:(NSArray *)results
{
    ...
    [keychain synchronize];//注册成功后才将用户名密码写入NSUserDefaults
...
{
    [[WZKeychain keychain] invalid];//注册失败不将用户名密码写入NSUserDefaults，并清除keyChain
