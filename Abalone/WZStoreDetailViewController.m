...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.storeNameLabel.text = self.merchant.name;
    self.storeNameLabel.backgroundColor = [UIColor blackColor];
    //self.storeImageView.imageURL = [NSURL URLWithString:self.store.vipImage];
    
    
  
      NSArray *chunks = [self.merchant.url componentsSeparatedByString: @","];
    
    
    self.storeImageView.images = chunks;
    self.tableView.tableHeaderView = self.storeImageView;
    
    self.storeDetailInfoViewController.merchant = self.merchant;
    self.serviceItemListViewController.merchant = self.merchant;
    [self.serviceItemListViewController viewWillAppear:animated];

    self.commentListViewController.merchant = self.merchant;
    self.storeAdListViewController.merchant = self.merchant;
    
    [self.storeAdListViewController viewWillAppear:animated];
    
    [self.merchant fetchMerchant];
    [self updateViews];
}
...
-(void)fetchMerchantSuccess:(NSNotification *)notification
{
    ...
    self.storeNameLabel.text = merchant.name;
...
-(void)updateViews
{
    ...
    NSArray *chunks = [self.merchant.image componentsSeparatedByString: @","];
    
    
    self.storeImageView.images = chunks;
    //self.storeImageView.imageURL = [NSURL URLWithString:];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"showServiceItemInfo"]) {
        if ([segue.destinationViewController isKindOfClass:[WZShowServiceItemDetailViewController class]]) {
            ...
            showServiceItemDetailViewController.merchant = self.merchant;
        } 
        ...
        if ([segue.destinationViewController isKindOfClass:[WZAddCommentViewController class]]) {
            ...
            addCommentViewController.merchant = self.merchant;
        }
        ...
        if ([segue.destinationViewController isKindOfClass:[WZMapViewController class]]) {
            ...
            //mapctrl.location = self.merchant.l;
        }
    }
}
